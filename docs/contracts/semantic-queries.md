...
derived capabilities, closure protocols, opaque-result bounds, iterator cursor
proofs, unsafe regions, and complete sugar expansion remain the later §22.5
queries associated with their semantic milestones. The same snapshot type can
grow those structured views without changing source identity or asking an LSP
to reinterpret syntax.
