...
This block does not choose a backend, define the public ABI, measure linked
binary size, or close Gate N1. Those concerns remain the following AOT blocks:
`NATIVE-AOT-BINARY-001`, `NATIVE-AOT-MEM-001`, `NATIVE-AOT-QUALITY-001` and
`NATIVE-AOT-PERF-001`.
