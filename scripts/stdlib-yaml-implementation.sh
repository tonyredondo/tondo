...
logs_dir="$evidence_dir/stdlib-yaml-implementation-logs"
...
TONDO_STDLIB_YAML_CONTRACT="$contract" scripts/stdlib-yaml-implementation-check.sh
...
CARGO_TARGET_DIR="$target_dir" cargo run -q -p tondo-cli --locked -- \
    run tests/runtime/m11-std-yaml-impl-001.to \
...
CARGO_TARGET_DIR="$target_dir" cargo test -q -p tondo-stdlib --locked \
    >"$stdlib_test_log" 2>&1 \
    || { cat "$stdlib_test_log" >&2; die "YAML stdlib tests failed"; }

compiler_test_log="$logs_dir/compiler-tests.log"
CARGO_TARGET_DIR="$target_dir" cargo test -q -p tondo-compiler --locked \
    'process_host::tests::yaml_' -- --nocapture \
    >"$compiler_test_log" 2>&1 \
    || { cat "$compiler_test_log" >&2; die "hosted YAML tests failed"; }
...
fixture_sha256="$(sha256sum tests/runtime/m11-std-yaml-impl-001.to | cut -d" " -f1)"
...
jq -n \
    ...
    '
      {
        format:"tondo-stdlib-yaml-implementation-evidence/1",
        task:"STD-YAML-IMPL-001",
        status:"passed",
        source_revision:$revision,
        contract_sha256:("sha256:" + $contract_sha256),
        vm:{fixture:"tests/runtime/m11-std-yaml-impl-001.to",fixture_sha256:("sha256:" + $fixture_sha256),exit:0,stdout:"yaml-ok",status:"passed",log_sha256:("sha256:" + $vm_sha256)},
        hosted_tests:{filter:"process_host::tests::yaml_",status:"passed",log_sha256:("sha256:" + $compiler_test_sha256)},
    ...
    ' >"$evidence_dir/stdlib-yaml-implementation.json"

echo "std.yaml implementation: OK (hosted VM and YAML scalar kernel; report: $evidence_dir/stdlib-yaml-implementation.json)"
