...
evidence_dir="${TONDO_STDLIB_SYNC_COLLECTION_PERF_EVIDENCE_DIR:-$root/target/reliability/evidence}"
target_dir="${CARGO_TARGET_DIR:-target}"

die() {
    echo "std.sync collection performance: $*" >&2
    exit 1
}
...
jq -e '
    ...
    and .protocol == {
        ...
        batch_operations: 16,
        deterministic_seed: "tondo-stdlib-sync-collection-perf-0.1",
    ...
    and ([.metrics[]] | sort) == [
        "allocations", "latency", "live_handles", "logical_memory_bytes",
        "parking", "retries", "tail_latency", "throughput", "wakeups"
    ]
    ...
    and all(.workloads[];
        ...
        and (.collection == "array" or .collection == "map" or .collection == "set" or .collection == "stack" or .collection == "queue")
        and (.operation | test("^[a-z0-9-]+$"))
        and (.mode | test("^[a-z0-9-]+$"))
        and (.participants == 1 or .participants == 8 or .participants == 64)
        and (.cardinality == 1 or .cardinality == 8 or .cardinality == 64)
    )
    and .strategy.hosted_vm == "single-worker-ready-job-collection-baseline"
    ...
    and .invariants.participants == "logical-execution-units-1-8-64; hosted-execution-is-single-worker"
    and .invariants.contention == "hosted-collection-calls-complete-ready-with-zero-retries-wakeups-and-parking"
    and .invariants.allocations == "logical-host-value-allocations-including-fixture; allocator-overhead-excluded"
    and .invariants.memory == "peak-logical-collection-payload-generations-and-host-registry; RSS-excluded"
    and .invariants.cursor == "direct-next-has-no-content-materialization-or-visited-table-and-does-not-hold-body-lock"
    and .invariants.snapshot == "one-coherent-result-per-linearization-point"
    and .invariants.resize == "growth-is-bounded-by-cardinality-and-never-exceeds-host-limit"
    and .invariants.cleanup == "no-pending-collection-job-waiter-or-scheduler-queue-before-probe-return"
    and .invariants.oracle == "independent-bounded-collection-model-plus-host-observable-outcome-checks"
...
' "$contract" >/dev/null || die "invalid machine-readable collection performance contract"
...
if [[ "${TONDO_STDLIB_SYNC_COLLECTION_PERF_ALLOW_DIRTY:-0}" != 1 ]]; then
    [[ -z "$(git status --porcelain)" ]] || die "workspace must be clean"
fi

mkdir -p "$root/.tmp"
tmp="$(mktemp -d "$root/.tmp/tondo-stdlib-sync-collection-performance.XXXXXX")"
trap 'rm -rf -- "$tmp"' EXIT
...
CARGO_TARGET_DIR="$target_dir" cargo test -p tondo-reliability --test sync_collection_models --locked \
    > "$oracle_log" 2>&1 || {
    cat "$oracle_log" >&2
    die "independent collection oracle failed"
}
...
for process in 1 2 3; do
    ...
    grep -F $'TONDO_SYNC_COLLECTION_PERF\t' "$log" >> "$samples" \
...
jq -Rn \
    ...
    '
      [inputs
       | split("\t")
       | select(length == 14 and .[0] == "TONDO_SYNC_COLLECTION_PERF")
       | {
           workload_id: .[1],
           collection: .[2],
           mode: .[3],
           participants: (.[4] | tonumber),
           cardinality: (.[5] | tonumber),
           nanos: (.[6] | tonumber),
           operations: (.[7] | tonumber),
           allocations: (.[8] | tonumber),
           retries: (.[9] | tonumber),
           wakeups: (.[10] | tonumber),
           parking: (.[11] | tonumber),
           logical_memory_bytes: (.[12] | tonumber),
           live_handles: (.[13] | tonumber)
       }] as $rows
      ...
      | ($rows | group_by(.workload_id) | map(
          ...
          | {
              ...
              counters: {
                ...
                retries: ($group | map(.retries) | unique[0]),
                wakeups: ($group | map(.wakeups) | unique[0]),
                parking: ($group | map(.parking) | unique[0]),
              ...
              dimensions: {
                ...
                retries: {unit: "attempts", count: ($group | map(.retries) | unique[0])},
                wakeups: {unit: "events", count: ($group | map(.wakeups) | unique[0])},
                parking: {unit: "events", count: ($group | map(.parking) | unique[0])}
              }
            }
      )) as $measurements
      | {
          format: "tondo-stdlib-sync-collection-performance-report/1",
          edition: "0.1",
          phase: "STD-0.1B",
          task: "STD-SYNC-COLLECTION-PERF-001",
          suite: "tondo-stdlib-sync-collection-performance",
          owner: "std.sync.collection",
    ...
    ' < "$samples" > "$evidence_dir/stdlib-sync-collection-performance.json"
...
jq -e \
    ...
    '
      ...
      and all(.measurements[];
          ...
          and .counters.parking == 0
          and .dimensions.tail_latency.p95 >= .dimensions.latency.median
          and .dimensions.tail_latency.p99 >= .dimensions.tail_latency.p95
      )
    ' "$evidence_dir/stdlib-sync-collection-performance.json" >/dev/null \
    || die "captured report failed validation"

echo "std.sync collection performance: OK (31 workloads; 27 samples each; report: ${evidence_dir#"$root"/}/stdlib-sync-collection-performance.json)"
