...
jq '.status = "closed" | .implemented_blocks = ["ARC-001", "ARC-002"] | .pending_blocks = [] | .next_blocks = ["NATIVE-STD-HOSTED-001"] | .corpus.arc_002_cases = .corpus.arc_002_cases[0:2]' testing/native-arc.json > "$tmp/missing-cycle-case.json"
