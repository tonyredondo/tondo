...
jq -n \
    --arg manifest_sha256 "$(sha256sum conformance/0.1/manifest.json | cut -d' ' -f1)" \
    ...
    | ($matrix.owners | sort_by(.id) | map(
        ...
        | {
            ...
            layer: $owner.layer,
            status: "pending",
            reason: "Declared cases require a current execution report; generation does not observe execution",
    ...
    | {
        ...
        runner: {
          ...
          full_suite_case_count: ($suite[0].cases | length),
        ...
        rules: {
          every_matrix_row_has_case_plan: true,
          ...
          generation_does_not_promote: true
        },
        owners: $owners,
        summary: {
