...
const INVALID_DIRECT_AWAIT_SOURCE: &str = r#"fn compute(): Int suspends {
    42
}
...
#[test]
fn direct_wait_has_one_stable_suspends_interface_hash() {
...
#[test]
fn explicit_await_rejects_a_direct_call_with_e1611() {
    let explicit = observation(source_request(
        "suspension-explicit-direct-call",
        WireOperation::Check,
        WireSourceForm::Module,
        INVALID_DIRECT_AWAIT_SOURCE,
        false,
    ));
    assert_eq!(explicit.compilation, CompilationState::Rejected);
    assert_eq!(
        explicit
            .diagnostics
            .iter()
...
#[test]
fn sync_boundaries_reject_inferred_suspension_with_e1601() {
